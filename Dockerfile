...
RUN apt-get update -y && \
  apt-get install -y libstdc++6 openssl libncurses5 locales ca-certificates chromium ghostscript wget \
  && apt-get clean && rm -f /var/lib/apt/lists/*_*
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/phoenix_vault ./
